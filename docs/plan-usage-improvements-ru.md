...
## 9. Статус реализации (ветка `arena/01a0cfaa-real`, 23.09.2026)
...
| 2. Обновления | сделано | Режимы `updates.mode`: `off` (по умолчанию — ни одного сетевого запроса) и `manual` (проверка только по команде из меню или `--check-updates`), приложение никогда не закрывается и не обновляется само; HTTP переведён на WinHTTP, libcurl удалён из дерева |
...
| 4. Переинициализация | сделано | Вручную: меню трея, кнопка в окне, `Ctrl+Alt+R`, `REAL.exe --reinit`. Автоматически: смена устройства по умолчанию, изменение состояния, добавление/удаление устройства (по умолчанию выключено), выход из сна, разблокировка сеанса, плюс периодическая проверка раз в 30 секунд (жив ли поток, то ли устройство). Всё в одном процессе, без перезапуска |
| 5. Windows 11 | частично | Готово: расшифровка HRESULT (`AUDCLNT_E_*`), обход `AUDCLNT_E_ENGINE_PERIODICITY_LOCKED`/`_FORMAT_LOCKED` через принятие текущего периода, снятие power throttling, управление приоритетом процесса, понятное сообщение при отсутствии `IAudioClient3` (Bluetooth/виртуальные драйверы), подробный лог. Осталось (по желанию): диагностический отчёт «устройство/драйвер/периоды» одной командой, автовыход, если выигрыша нет, MMCSS, ограничение affinity (экспериментально, требует прав) |
