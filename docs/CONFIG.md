...
пишутся сразу на нужном языке. После смены языка приложения существующий файл
**не переписывается**: комментарии остаются на языке создания файла, а сам
интерфейс и журнал переключаются при следующей перезагрузке настроек.
